    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>Secretaries</v-toolbar-title>
          <v-divider
            class="mx-4"
            inset
            vertical
...
export default {
  ...
  computed: {
    ...
    dialogTitle() {
      return this.isUpdateTask ? "Update Secretary" : "Register Secretary";
    },
    ...mapGetters({
      secretaries:    'secretary/getSecretaries',
      secretary:      'secretary/getSecretary',
      secretaryIndex: 'secretary/getIndex',
  ...
  methods: {
    ...
    storeSecretary() {
      return store.dispatch('secretary/createSecretary', this.getFormData())
        .catch(error => {
    ...
    updateSecretary() {
      return store.dispatch('secretary/updateSecretary', this.getFormData())
        .catch(error => {
          this.serverErrors = error.response.data.errors.detail
          throw error
        });
    ...
    deleteSecretary() {
      return store.dispatch('secretary/deleteSecretary')
        .catch(
    ...
    editItem(item) {
      store.dispatch('secretary/setIndex', this.secretaries.indexOf(item))
    ...
    deleteItem(item) {
      ...
      confirm("Are you sure?") && this.deleteSecretary()
    ...
    endTask() {
      ...
      this.serverErrors = {}
      store.dispatch('secretary/unsetIndex')
    },

    save() {
      
      let promise = this.isUpdateTask ? this.updateSecretary() : this.storeSecretary()
